...
def Rename_BSE_Bhavcopy_Files(directory):
    ...
    for file in files:
        if file.endswith(".csv"):
            ...
            date = file[-12:-4]
            new_name = date + ".csv"
            new_name = new_name.replace("BSE_EQ_BHAVCOPY_", "")
...
def Modify_BSE_Bhavcopy_Files(directory):
	...
	"Settlement Price",
                ...
                df = df[~df['SCRIP ID'].isin(Mutual_Fund_symbol_to_remove)]
                ...
                cols.insert(cols.index('OPEN PRICE'), 'TRADING_DATE')
                df = df[cols]
...
for date in date_range:
    ...
    #https://www.bseindia.com/download/BhavCopy/Equity/BSE_EQ_BHAVCOPY_29122023.ZIP
    BSE_Bhavcopy_URL = "https://www.bseindia.com/download/BhavCopy/Equity"
    date_str = date.strftime("%d%m%Y")
    filename = f"BSE_EQ_BHAVCOPY_{date_str}.ZIP"
